...
>
		...
		>
			...
			<div class="space-y-1">
				...
				<p class="text-sm text-muted-foreground">
					We've sent a magic link to <strong class="text-normal">{$formData.email}</strong>
				</p>
			</div>
		</div>
	{/if}
	...
	<Form.Field {form} name="email">
		...
		<Form.FieldErrors class="text-sm text-destructive mt-1" id="email-error" role="alert" />
		{#if emailSent}
	...
	<div class="text-center text-sm text-muted-foreground">
		<p>
			...
			and
			<Button variant="link" href="/privacy" class="px-1 py-0 h-auto font-medium">
				Privacy Policy
			</Button>
		</p>
	</div>
